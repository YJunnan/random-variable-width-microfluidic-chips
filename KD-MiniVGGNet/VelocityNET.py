...
data = pd.read_csv('RVW.csv', header=None)
#X = joblib.load('randomall2D7225_shuffle.pkl')
data1 = pd.read_csv('RVW.csv', header=None)
physics = 'velocity'
Y = data.iloc[:, 84:87].to_numpy()  # concentration
#Y = Y+1
...
#np.savetxt('test_X_V.csv', test_X.reshape(2558,81), delimiter = ',')
#np.savetxt('test_y_V.csv', test_y.reshape(2558,3), delimiter = ',')
#np.savetxt('train_X_V.csv', train_X.reshape(7674,81), delimiter = ',')
#np.savetxt('train_y_V.csv', train_y.reshape(7674,3), delimiter = ',')
test_X = (test_X).cuda()
test_y = (test_y).cuda()
...
class GooGleNet(nn.Module):
    def __init__(self, num_classes=3, init_weights=False):
        ...
        self.features = nn.Sequential(  #打包
            ...
            # nn.BatchNorm2d(128),
            #
            # nn.Conv2d(128, 128, kernel_size=(2, 2), stride=(1, 1), padding=(0, 0)),
            # nn.ReLU(inplace=True),
            # nn.BatchNorm2d(128),
            # nn.MaxPool2d(kernel_size=2, stride=2),
        )
