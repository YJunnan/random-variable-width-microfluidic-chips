...
Y = data.iloc[:, 83:84].to_numpy()  # concentration
...
print(Y)
#加载数据
train_X, test_X, train_y, test_y = ms.train_test_split(X, Y, test_size=0.25, random_state=2)
...
def train(eopchs):
    ...
    for steps, (batch_x, batch_y) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
        #train_accuracy = torch.mean((output - torch.mean(batch_y)) ** 2) / torch.mean((batch_y - torch.mean(batch_y)) ** 2)
        train_accuracy = (batch_y-output) / output#(batch_y-torch.mean(batch_y))**2/(output-torch.mean(batch_y))**2
...
def test(eopchs):
    ...
    with torch.no_grad():
        for test_batch_x, test_batch_y in test_loader:
            ...
            #test_accuracy = torch.mean((test_prediction - torch.mean(test_batch_y)) ** 2) / torch.mean((test_batch_y - torch.mean(test_batch_y)) ** 2)
